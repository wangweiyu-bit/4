...
(function(){
  ...
  function getBasePrefix(){
    const path = location.pathname.replace(/\\+/g,'/');
    const segments = path.split('/').filter(Boolean); // e.g. ["pages","code.html"]
    // depth 1 => at root (index.html, 404.html, etc.)
    // depth >1 => in subdirectory like /pages/
  ...
  async function guardPageAccess(){
    ...
    if(!apiState.mockMode){
      try{
        ...
        if(!me || !me.approved){ alert('需要管理员授权后才能访问'); location.href = 'index.html'; }
